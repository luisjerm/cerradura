/*
* MFRC522.c  - Modification of the MFRC522.cpp library to use ARDUINO RFID MODULE KIT 13.56 MHZ WITH 
* TAGS SPI W AND R BY COOQROBOT to be used in C.
...
#define FALSE 0
#define TRUE 1
typedef unsigned char PinName;

// Member variables
  Uid uid;                // Used by PICC_ReadCardSerial().
 
  // Size of the MFRC522 FIFO
  const uint8_t FIFO_SIZE = 64;   // The FIFO is 64 bytes.
...
void PCD_WriteRegister(uint8_t reg, uint8_t value)
{
  ...
  (void) m_SPI_write(reg & 0x7E);
  (void) m_SPI_write(value);
...
void PCD_ReadRegister_2(uint8_t reg, uint8_t count, uint8_t *values, uint8_t rxAlign)
{
  ...
  (void) m_SPI_write(address);   // Tell MFRC522 which address we want to read
  ...
  while (index < count)
  {
    ...
    {
      // Read value and tell that we want to read the same address again.
      values[index] = m_SPI_write(address);
    }
 
    index++;
  }
 
  values[index] = m_SPI_write(0); // Read the final byte. Send 0 to stop reading.
...
uint8_t PCD_CalculateCRC(uint8_t *data, uint8_t length, uint8_t *result)
{
  ...
  PCD_WriteRegister_2(FIFODataReg, length, data); // Write data to the FIFO
...
uint8_t PCD_MIFARE_Transceive(uint8_t *sendData, uint8_t sendLen, bool acceptTimeout)
{
  ...
  uint8_t waitIRq ;    // RxIRq and IdleIRq
